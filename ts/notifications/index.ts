...
export const notify: Notify = (options, componentId?): void => {
  const event = new CustomEvent('bladeui:notification', { detail: { options, componentId } })
...
export const confirmNotification: Confirm = (options, componentId?): void => {
  ...
  const event = new CustomEvent('bladeui:confirm-notification', { detail: { options, componentId } })
  window.dispatchEvent(event)
}
